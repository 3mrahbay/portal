...
import {yetkili,kayitDogrula,oncelik,geciken,raporHTML,tarihGecerli,aktifDonemOgrencisi} from '../js/pdr/core.js';
...
test('Aktif dönem filtresi eski dönem ve arşiv öğrencilerini dışlar',()=>{
 assert.equal(aktifDonemOgrencisi({aktifDonem:'2026-2027',aktifDonemDurum:'aktif'},'2026-2027'),true);
 for(const o of [{},{aktifDonem:'2025-2026',durum:'aktif'},{aktifDonem:'2026-2027',aktifDonemDurum:'arsiv'}])assert.equal(aktifDonemOgrencisi(o,'2026-2027'),false);
});

test('Kural regex dolar işareti kaynak ekini çoğaltmaz',()=>{
 const snippet=readFileSync(new URL('../security/pdr.rules.snippet',import.meta.url),'utf8');
 const source="match /duyurular/{id} {}\nmatch /etkinlikler/{id} {}\nmatch /takvim/{id} {}\n    // ====== PDR — ÖZEL NİTELİKLİ KİŞİSEL VERİ (KVKK md.6) ======\n// UNIQUE_SUFFIX\nmatch /{collection}/{document=**} { allow read, write: if isAdmin(); }";
 const result=prepare(source,snippet);
 assert.equal(result.split('UNIQUE_SUFFIX').length,2);
 assert.ok(result.includes(snippet));
 assert.ok(result.includes("collection != 'pdrTakipKayitlari'"));
});
