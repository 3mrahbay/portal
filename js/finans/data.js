...
export async function okulVerisi({fb,db,donem}){
  ...
  const ogrenciler=(await sinirliMap(students.docs,async d=>{const ds=await fb.getDoc(fb.doc(db,'ogrenciler',d.id,'donemler',donem));if(!ds.exists())return null;const o=d.data(),v=ds.data();return {id:d.id,ad:o.ogrenciAdSoyad||o.ad||d.id,sinif:v.kayit?.sinif||o.sinif||'',kayitTarihi:kayitZamani(v,o),veli:v.anne?.adSoyad||v.baba?.adSoyad||v.veli?.adSoyad||o.veliAdSoyad||o.anneAdSoyad||o.babaAdSoyad||'',veri:v,plan:odemePlani(v)};})).filter(Boolean);
