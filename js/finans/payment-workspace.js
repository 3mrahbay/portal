import {esc,para,bugun,bildirimKalemi,odemePlani,sirala,gecerliTarih} from './core.js';
...
function bildirimKartlari(notes){return sirala(notes,'odemeTarihi','desc').map(b=>`<article class="card payment"><div><b>${esc(b.kalemAd||b.ayAd||bildirimKalemi(b)||'Ödeme bildirimi')}</b><small>${para(b.bildirilenTutar??b.tutar)} · ${esc(b.odemeTarihi||'Tarih eksik')} · ${esc(labels[b.durum]||b.durum)}</small>${!b.donem?'<small class="finance-warning">Dönem bilgisi eksik; onaydan önce seçip doğrulayın.</small>':''}</div><div class="toolbar">${bekleyen(b)?`<button type="button" data-review="${esc(b.id)}">${b.donem?'Kontrol et / Onayla':'Dönemi doğrula / Onayla'}</button>`:''}${b.dekontVar?`<button type="button" data-receipt="${esc(b.id)}">Dekont</button>`:''}</div></article>`).join('')||'<p>Bu döneme ait bildirim yok.</p>';}
...
export async function hesapAc(root,ctx,{ogrenciId,donem,onDirtyClose=()=>{}}){
 ...
 function draw(){
  ...
  <section class="section"><h3>Kontrol bekleyen bildirimler (${pending.length})</h3><div data-notes>${bildirimKartlari(pending)}</div></section>
